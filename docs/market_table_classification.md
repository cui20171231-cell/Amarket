...
1. `scheduled_time` 必须是所属交易日15:00:00。
2. `ingest_time` 必须不早于15:00:00，证明该行是在收盘节点到达后落库。
3. 接口 `source_time` 最多只能比15:00早60秒，允许三池正常返回14:59:45等最终时间戳，
   但拒绝14:56等上一节点来源。
...
3. `calculated_at` 或派生落库时间必须不早于15:00:00。
4. 三类板块状态的板块指数来源不能为空。
